...
볼륨(`docker volume create`)을 사용하면 컨테이너가 삭제되어도 데이터가 유지됩니다. 컨테이너는 일회용 실행 환경이고, 볼륨은 Docker가 별도로 관리하는 영구 저장소이기 때문입니다. 데이터가 중요한 경우 반드시 볼륨 또는 바인드 마운트로 호스트와 연결해야 합니다.
